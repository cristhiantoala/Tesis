...
with open(r'textFiles\emotion_level.txt') as f:
    for line in f:
...
with open(r'textFiles\mouse_log.txt') as f:
    for line in f:
        result = regex.findall(line)
...
with open(r'textFiles\key_log.txt') as f:
    for line in f:
        result = regex.findall(line)
...
for ite in range(0, len(stressLogM)):
    ...
    for num, l in enumerate(keyLogM, start=saveNum3):
        if start4 == 1 and correctNum3 == 1:
            date2 = datetime.datetime.strptime(str(keyLogM[num][2]), '%H:%M:%S,%f').time()
            time2 = date2.hour * 60 * 60 + date2.minute * \
				60 + date2.second + date2.microsecond / 1000000
...
f.close()
f = open(r'textFiles\metricsNew.txt', "a")
